...
timing.comp= function(dat, analysis_type, missing.dat){
  ...
  if (analysis_type =="CC"){ #dat is observed data
    time1.mod = lm(logbinge1~A1,data=dat)
    ...
    fit = lapply(1:max(dat$.imp),function(x){lm(logbinge1~A1,data=dat[dat$.imp==x,])})
  ...
  res$HD.prop = prop.table(table(dat$logHD))[2]
  res$miss.rateTV = prop.table(table(is.na(missing.dat$logsm_binge_last)))[2] #missingness rate of TV
  res$miss.rateY = prop.table(table(is.na(missing.dat$logbinge1)))[2] #missingness rate of binge1 (outcome)
