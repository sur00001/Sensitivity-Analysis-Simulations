...
induce.missYt = function(nperson,t,yt,wide.df,alpha_m=NULL, beta_y=NULL, beta_by=NULL,beta_hs=NULL,
                         ...
                         beta_hd=NULL,beta_a2=NULL,beta_a2logbinge1= NULL,
                         beta_a1logbinge1=NULL,beta_smb=NULL,beta_smh=NULL,beta_a1smb=NULL,
  ...
  if(miss_type=="MAR"){ 
    if (t<1){ #tailoring variable
      ...
      yt.missprob = expit(alpha_m+
                            beta_y*(wide.df$logbinge0-mean(wide.df$logbinge0))+
                            beta_a1*(wide.df$A1-mean(wide.df$A1))+
  ...
  if(miss_type=="MNAR"){ 
    if (t<1){ #tailoring variable 
      yt.missprob = expit(alpha_m+(beta_y/2)*(wide.df$logsm_binge_last-mean(wide.df$logsm_binge_last))+
                            ...
                            beta_a1*(wide.df$A1-mean(wide.df$A1)) + 
                            beta_a1smb*((wide.df$A1-mean(wide.df$A1))*(wide.df$logsm_binge_last-mean(wide.df$logsm_binge_last)))) #interaction with trt and sm value
      ...
      yt.missprob = expit(alpha_m+beta_a1*(wide.df$A1-mean(wide.df$A1)) + beta_a2*(wide.df$A2-mean(wide.df$A2)) +
                            ...
                            beta_a1logbinge1*((wide.df$A1-mean(wide.df$A1))*(wide.df$logbinge1-mean(wide.df$logbinge1)))+ 
                            beta_a2logbinge1*((wide.df$A2-mean(wide.df$A2))*(wide.df$logbinge1-mean(wide.df$logbinge1))))
  }
